...
\ac{PE} diagnostic prediction. AWS SageMaker was also used to
...
This was a retrospective, descriptive, cross-sectional observational
study that used an anonymized database of medical records from
...
clinical data that we produced. For the former, we trained our models using AWS SageMaker Canvas and our proposed methodology \ac{C-AutoML}. For the
...
formed by \texttt{LightGBM}, \ac{MLP}, and \ac{RF} with an $\mathrm{F1}=0.805$,
$\mathrm{Precision}=0.770$, $\mathrm{Recall}=0.844$, and
...
\end{adjustwidth}
\caption{\ac{SHAP} explainability visualizations for the AWS SageMaker
...
to the controlled synthetic data generation. Nevertheless, the results identify \ac{LR} as a suitable model for a \ac{PIERS} application.
...
models within the computing infrastructure of the \ac{EPN}. The system is currently accessible within the \ac{EPN} campus network and exposes the best-performing models produced by the \ac{C-AutoML} workflow and AWS SageMaker Canvas procedure as a prediction service.
