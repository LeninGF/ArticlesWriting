...
The implementation requires the following Python packages with specified compatibility:
\begin{itemize}
    \item \texttt{scikit-learn} $\ge$ 1.0.0 (primary machine learning framework)
    \item \texttt{pandas} $\ge$ 1.3.0 (data manipulation)
    \item \texttt{numpy} $\ge$ 1.21.0 (numerical computation)
    \item \texttt{matplotlib} $\ge$ 3.4.0 (visualisation)
    \item \texttt{joblib} $\ge$ 1.0.0 (model serialisation)
\end{itemize}
Optional packages for extended model support:
\begin{itemize}
    \item \texttt{xgboost} $\ge$ 1.5.0 (gradient boosting)
    \item \texttt{lightgbm} $\ge$ 3.3.0 (gradient boosting)
    \item \texttt{catboost} $\ge$ 1.0.0 (gradient boosting)
\end{itemize}
