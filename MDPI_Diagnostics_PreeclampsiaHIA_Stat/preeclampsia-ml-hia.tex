...
\added[id=R2]{Clinically, \ac{PE} is defined as new-onset hypertension
(systolic blood pressure $\geq$140~mmHg or diastolic $\geq$90~mmHg)
after 20 weeks of gestation, accompanied by proteinuria or evidence of
maternal end-organ damage~\cite{brown2018isshp}. International guidelines
further distinguish early-onset \ac{PE} (onset before 34 weeks of
gestation) from late-onset \ac{PE} (at or after 34 weeks), which differ
in placental pathophysiology, severity profile, and associated neonatal
morbidity~\cite{brown2018isshp}. Presentations accompanied by markedly elevated blood pressures, thrombocytopenia,
renal insufficiency, hepatic dysfunction, pulmonary oedema, or new-onset
headache are classified as having severe features, indicating a higher-risk
clinical course requiring urgent management~\cite{brown2018isshp}.}
...
SageMaker Canvas is a managed no-code machine-learning service that
...
The proposed methodology fits full \texttt{scikit-learn} pipelines with
...
on model \texttt{LightGBM}, with an $\mathrm{F1}=0.802$,
...
performance was achieved by the \texttt{Weighted Ensemble Model}.
...
$\mathrm{AUC}=0.956$. Compared to the \texttt{LightGBM} model trained with
...
the precision of the \texttt{LightGBM} model implies a smaller
...
for the \texttt{Weighted Ensemble} model trained in Amazon SageMaker
...
\texttt{LightGBM} ($\mathrm{Recall}=0.768$). This is important,
...
($\mathrm{Precision}=0.837$) compared to the \texttt{Weighted Ensemble
...
with \ac{LR}, \ac{RF}, \ac{MLP}, \texttt{XGBoost}, \texttt{LightGBM},
and \texttt{CatBoost} all reaching $\mathrm{F1}=1.000$ in the first
...
\texttt{Weighted Ensemble} model trained in Amazon SageMaker Canvas achieved an
...
given that the \texttt{Weighted Ensemble} model provides a
