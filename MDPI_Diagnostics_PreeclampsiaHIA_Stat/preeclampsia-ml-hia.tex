...
\Author{María Pérez $^{1,}$*\orcidA{}, Lenin G. Falconi $^{1}$\orcidB{},
  Monserrate Intriago-Pazmiño $^{1}$\orcidC{}, Andrés Bastidas-Fuertes $^{2}$\orcidB{}, and Juan Benavidez $^{1}$\orcidE{}}

\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{María Pérez, Lenin G. Falconi, Monserrate
  Intriago-Pazmiño, Andrés Bastidas-Fuertes, and Juan Benavidez}

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
$^{1}$ \quad Escuela Politécnica Nacional, Facultad de Ingeniería en Sistemas, 170525, Quito, Ecuador; maria.perez@epn.edu.es, lenin.falconi@epn.edu.ec, 
...
\abstract{Preeclampsia is a major contributor to maternal and
  ...
  demographic data from the Isidro Ayora Hospital in Quito,
...
\begin{acronym}
    ...
    \acro{F1}[F1]{F1-score}
    \acro{MSP}[MSP]{Ministry of Public Health}
...
Preeclampsia, one of the main hypertensive disorders of pregnancy, is
a major cause of maternal and perinatal morbidity and mortality
worldwide. The World Health Organization reports that preeclampsia
...
In Ecuador, the Clinical Practice Guideline from the \ac{MSP} identifies
preeclampsia and eclampsia among the principal causes of maternal
...
% improve with prism
In recent years, \ac{ML} has been increasingly applied to preeclampsia
...
preeclampsia~\cite{fullpiers_2011}. It uses the logistic regression
...
research, we used the provided records from the \textit{Isidro Ayora
  Third Level Gynecology and Obstetrics Hospital}\footnote{Hospital
  Gineco Obstétrico Isidro Ayora} in Quito, Ecuador; which contains
demographic variables and lacks of clinical features. For this reason,
we explored our sets of models both on the real and synthetic data to
diagnose preeclampsia. Different to the full\ac{PIERS} standard, which
is a prognostic model, our goal is to determine the most effective set
of models to diagnose preeclampsia according to the Ecuadorian realm.
...
preeclampsia diagnostic prediction. AWS Sage Maker was also used to
...
learning algorithms in preeclampsia in Ecuador, (ii) we systematically
...
used data provided from \textit{Isidro Ayora Third Level Gynecology
  and Obstetrics Hospital}; which is a specialized public health
facility in gynecology and obstetrics, (iv) we explored additional
models on synthetic data to account for lack of clinical variables,
(v) we deployed our model using a \ac{MLOPs} approach in a server at
Escuela Politecnica Nacional.

\section{Related Works}
\label{sec:related-works}
Recent literature shows sustained interest in using \ac{ML} for
preeclampsia prediction, but also highlights important differences in
...
random forest, with AUC values ranging approximately from 0.860 to
...
diversity of preeclampsia prediction strategies. Marin et
al.\cite{marin2019} used the Viterbi algorithm to determine the
presence of preeclampsia based on features such as the weight, age,
...
risk of preeclampsia from electronic health records collected in early
pregnancy. The XGBoost model achieved the best performance
($\ac{ACC}=0.920$, $\mathtt{Precision}=0.447$, $\mathtt{Recall}=0.789$,
...
and \ac{BMI}, can provide useful discriminative power for preeclampsia
...
preeclampsia ($\ac{AUC}=0.83$) using a 9‑metabolite urine model, with
...
preeclampsia prediction models tend to perform best when they combine
...
interpretable preeclampsia modeling in a real public-hospital
environment.
...
preeclampsia in women at Isidro Ayora Hospital. We restricted our
...
pregnant woman, the probability that a patient has preeclampsia is modeled
as:

\begin{linenomath}
\begin{equation}
  \label{eq:preeclampsiaClassification}
  P(\text{Preeclampsia} = 1 \mid \mathbf{x}) = f(\mathbf{x}; \boldsymbol{\theta})
\end{equation}
\end{linenomath}
...
\begin{equation}
  \hat{y} =
  \begin{cases}
    1 & \text{if } P(\text{Preeclampsia} = 1 \mid \mathbf{x}) \geq \tau \\
    0 & \text{otherwise}
  \end{cases}
...
Isidro Ayora Third Level Gynecology and Obstetrics Hospital in Zone 9
of the \ac{MSP} in Quito, Ecuador. Our research goal is to train a
machine learning classification model to predict preeclampsia. We
...
in compliance with ethical requirements approved by the Human Research
Ethics Committee of the Pontificia Universidad Católica del Ecuador.

\subsection{Data Preprocessing}
\label{sec:data-preprocessing}

Data preprocessing was designed to construct a reliable binary
classification dataset for preeclampsia prediction from routine
...
\item \textbf{Case identification:} We selected records associated with
  ...
  preeclampsia-related cases. Table~\ref{tab:cie10-icd10-mapping}
...
\item \textbf{Dataset assembly:} The preeclampsia and normal-pregnancy
...
\item \textbf{Target encoding:} A binary outcome variable was created
  ...
  pregnancies and \numprint{674} preeclampsia cases, as summarized in
...
\begin{table}[H]
  ...
  \caption{ICD-10 codes used to define the binary target variable for
    preeclampsia classification. The positive class (Preeclampsia = 1)
    indicates the presence of the disorder, while the negative class
    (Preeclampsia = 0) corresponds to normal pregnancy supervision
    codes.}
...
We have analyzed the prevalence of preeclampsia among the population
of pregnancies at the dataset provided by Isidro Ayora Hospital. Some
of the important insights are that preeclampsia prevalence rises
strongly with the patient's age: 85.13\%. The mean of women diagnosed
with preeclampsia present a mean \ac{BMI} of 33. Normal pregnancies
...
prevalence of preeclampsia in pregnancies at Isidro Ayora Hospital.
...
\begin{figure}[H]
  ...
  \caption{\label{fig:statistics}Statistical analysis of prevalence of
    preeclampsia in the population of pregnant women at Isidro Ayora
    Hospital. Preeclampsia is more frequent in women of 36 to 49 years
...
\begin{table}[H]
  ...
  preeclampsia diganosis in pregnant women at Isidro Ayora Hospital
...
pregnant patients with and without preeclampsia. The generation process was
...
This research aims to diagnose Preeclampsia with the provided data
from Isidro Ayora Hospital. In Section~\ref{sec:materials-methods}, we
defined the preeclampsia diagnose problem as a binary
...
experimentation and methodological traceability. 
...
higher values indicate better separation between preeclampsia and
non-preeclampsia classes~\cite{fawcett2006roc}.
...
\subfloat[Normalized confusion matrix for the best Weighted Ensemble
  ...
  \includegraphics[width=0.48\fulllength]{./figures/canvas_sagemaker_model_monitor/best_model_confusion_matrix_normalized.png}}
\subfloat[KernelSHAP feature importance values for the AWS SageMaker
  Weighted Ensemble.\label{fig:feature-importance-aws}]{
  \includegraphics[width=0.48\fulllength]{./figures/shap_explainability/shap-values-aws-ensemble.png}}\\
\subfloat[Precision--recall curve for the AWS SageMaker Weighted
  Ensemble model.\label{fig:precision-recall-aws}]{
  \includegraphics[width=0.48\fulllength]{./figures/canvas_sagemaker_model_monitor/best_model_precision_recall_curve.png}}
\subfloat[F1-score comparison across all models trained in AWS
  SageMaker Canvas.\label{fig:f1-score-models-isidro-ayora-aws}]{
  \includegraphics[width=0.48\fulllength]{./figures/canvas_sagemaker_model_monitor/all_models_f1_comparison.png}}
\end{adjustwidth}
...
probability of preeclampsia. \texttt{Gestational Age} also showed a
...
toward preeclampsia, while lower values (blue) tend to reduce
predicted risk.
...
For the \ac{LR} model trained on the clinical synthetic
dataset, the \ac{SHAP} summary shows a feature-importance structure that is
highly consistent with the rule-based and prognostic variables used to
construct the data. Diastolic blood pressure (DBP) and systolic blood
pressure (SBP) appear among the strongest positive contributors, with
higher values shifting the prediction toward preeclampsia.
Proteinuria at 24 hours, AST, creatinine, and the FullPIERS risk
percentage also display predominantly positive SHAP values at higher
feature levels, indicating that the model assigns greater risk when
these markers of renal, hepatic, and overall maternal severity
increase.

In contrast, higher platelet counts, oxygen saturation
(SpO2), and gestational age tend to move predictions toward lower
risk, which is reflected by more negative SHAP contributions at higher
values. Age shows a smaller but still coherent contribution, while
chest pain has a limited overall effect because of its lower
prevalence in the dataset. Taken together, these patterns suggest that
the \ac{LR} model learned a clinically interpretable
decision structure dominated by classical hemodynamic and
organ-dysfunction indicators of preeclampsia. This interpretation is
also reinforced by the \ac{SHAP} waterfall plot for the highest-risk
explained synthetic sample, in which AST, 24-hour proteinuria, low
SpO2, DBP, SBP, and the FullPIERS risk score produced the largest
positive contributions to the final model output, whereas chest pain
had a small negative contribution. The additive profile of this
instance-level explanation is therefore coherent with the global SHAP
ranking and shows that the Logistic Regression model identifies
high-risk cases through the accumulation of clinically severe
hemodynamic, renal, and hepatic abnormalities.
\section{Discussion}
\label{sec:discussion}

In this study we explored different models and approaches to train a
robust model for the prediction of preeclampsia. For that matter, we
...
of the actual population of women with preeclampsia, the model
...
preeclampsia. On the other hand, the \texttt{LightGBM} trained with
...
models towards the prediction of preeclampsia in the population of
...
preeclampsia cases correctly identified by the model, whereas the
...
$\mathtt{Precision}=0.837$. This means that 77\% of true preeclampsia
cases were correctly detected by the model. Consequently, this model
misses 1 out of 4 true preeclampsia cases. On the other hand, the
\texttt{Weighted Ensemble} model trained in AWS SageMaker achieved an
$\ac{F1}=0.805$, $\mathtt{Precision}=0.770$, and
$\mathtt{Recall}=0.844$. Therefore, 84\% of true preeclampsia cases
were correctly detected by the ensemble model and misses 1 out of 6
true preeclampsia cases. This improvement over our single model is due
to the ensemble model. We suggest a combination of both models models
to aid the physicians in their decision when diagnosing preeclampsia
