...
\Title{Preeclampsia Prediction at Isidro Ayora Hospital (ECU) using
  Machine-Learning}
...
\Author{María Pérez $^{1,}$*\orcidA{}, Lenin G. Falconi $^{1}$\orcidB{},
  Monserrate Intriago-Pazmiño $^{1}$\orcidC{}, Andrés Bastidas-Fuertes $^{2}$\orcidD{}, Juan Benavidez $^{1}$\orcidE{}, and Raysa A. Fuertes-Arévalo $^{3}$\orcidF{}}

\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{María Pérez, Lenin G. Falconi, Monserrate
  Intriago-Pazmiño, Andrés Bastidas-Fuertes, Juan Benavidez, and Raysa A. Fuertes-Arévalo}

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
$^{1}$ \quad Escuela Politécnica Nacional, Facultad de Ingeniería en Sistemas, 170525, Quito, Ecuador; maria.perez@epn.edu.ec, lenin.falconi@epn.edu.ec, 
monserrate.intriago@epn.edu.ec, 
juan.benavides@epn.edu.ec\\
$^{2}$ \quad Smartwork S.A.; andres.bastidas@smartwork.com.ec\\
$^{3}$ \quad Ministerio de Salud Pública (MSP), Quito, Ecuador; raysafuertesa@gmail.com}
...
%   workflow to evaluate six models (Logistic Regression, \texttt{LightGBM},
%   \texttt{XGBoost}, \texttt{CatBoost}, Multi-Layer Perceptron, and Random Forest) and
...
%   (\texttt{LightGBM}) achieved comparable results (F1-score: 0.802, accuracy:
...
\abstract{\textbf{Background:} Preeclampsia (PE) is a leading cause
  ...
  prediction modelling. This study aimed to develop a machine learning
  (ML) framework to predict PE using demographic data from the Isidro
  Ayora Hospital (HGIO), Quito, Ecuador.
  \textbf{Methods:} We proposed a custom automated machine learning
  (C-AutoML) workflow to evaluate six models (\texttt{Logistic Regression},
  \texttt{LightGBM}, \texttt{XGBoost}, \texttt{CatBoost}, \texttt{Multi-Layer
  Perceptron}, and \texttt{Random Forest}) and compared its performance against
  a weighted ensemble built with AWS SageMaker Canvas, optimizing for
  the F1-score. Model interpretability was analyzed using Shapley
  Additive Explanations (SHAP). Due to the scarcity of clinical
  variables at HGIO, a synthetic dataset based on FullPIERS predictors
  ...
  while the C-AutoML best single model (\texttt{LightGBM}) yielded
  ...
  \textbf{Conclusions:} The proposed C-AutoML workflow matched the
...
\begin{acronym}
    ...
    \acro{HGIO}[HGIO]{Isidro Ayora Gynecology and Obstetrics Hospital}
...
\Ac{PE}, one of the main hypertensive disorders of pregnancy, is
a major cause of maternal and perinatal morbidity and mortality
worldwide. The \ac{WHO} reports that \ac{PE}
affects approximately 2--8\% of pregnancies globally, highlighting the
relevance of early detection and risk stratification
strategies~\cite{who_2025_preeclampsia}.
...
using routinely collected variables~\cite{LI2021102,mennickent_2023}.
...
The full\ac{PIERS} model uses demographic and laboratory variables
(e.g. gestational age, creatinine, count of platelets, aspartate
...
calculated as linear combination of variables
...
\ac{HGIO}\footnote{\textit{Hospital Gineco Obstétrico Isidro Ayora (HGOIA)}, a
  third-level public specialty hospital in Zone~9 of the \ac{MSP},
  Quito, Ecuador.}; which contains demographic variables and lacks of
clinical features. For this reason, we explored our sets of models
both on the \ac{HGIO} and synthetic datasets to diagnose
\ac{PE}. Different to the full\ac{PIERS} standard, which is a
...
(\ac{LR}~\cite{cox1958logistic}, \texttt{LightGBM}~\cite{ke2017lightgbm},
\texttt{XGBoost}~\cite{chen2016xgboost},
...
\ac{PE} diagnostic prediction. AWS Sage Maker was also used to
evaluate the prediction power of the demographic variables of the
dataset of patients at \ac{HGIO} and to train a reference
...
results show that an ensemble model formed by \texttt{LightGBM}, \ac{RF} and
...
this is the first academic research work to address the use of machine
learning algorithms in \ac{PE} in Ecuador, (ii) we systematically
explored different machine learning models accounting to improve
classification performance as well as model's explainability, (iii) we
used data provided from data provided from \ac{HGIO}, a specialized
...
variables, (v) we deployed our model using a \ac{MLOPs} approach in a
...
gathered from \ac{HGIO} for our study. However, we
...
\ac{LR}, \ac{RF}, \ac{SVM}, and \texttt{XGBoost} models to predict women at
risk of \ac{PE} from electronic health records collected in early
pregnancy. The \texttt{XGBoost} model achieved the best performance
($\mathtt{\ac{ACC}}=0.920$, $\mathtt{Precision}=0.447$, $\mathtt{Recall}=0.789$,
$\ac{F1}=0.571$, $\ac{AUC}=0.955$) and showed that routinely available
...
\ac{PE} ($\ac{AUC}=0.83$) using a 9‑metabolite urine model, with
performance improving to $\ac{AUC}=0.94$ when integrating multiple
...
Third, despite strong reported predictive metrics, many studies are
developed in data-rich environments and depend on comprehensive
...
a descriptive analysis of retrospective patient records from \ac{HGIO}, (ii) a cloud-managed benchmarking workflow, and (iii) a
...
\ac{PE} in women at Isidro Ayora Hospital (HGIO). We restricted our
...
This was an observational and cross-sectional descriptive study that
used an anonymized database of medical records from patients at the
\ac{HGIO} in Quito, Ecuador. Our research goal is to train a
machine learning classification model to predict \ac{PE}. We
...
The records provided by \ac{HGIO} contain a total of \numprint{174753} records 49
descriptive variables (features) from year 2021 to 2025. However, the
dataset contains information about all the different diseases treated
at the hospital according to \ac{ICD-10}.
...
\item \textbf{Data quality profiling in AWS SageMaker Canvas:} We used
  Data Wrangler in SageMaker AI to assess data quality. The profiling
  ...
  dataset-quality outputs reported by SageMaker Canvas.
    ...
    \begin{tikzpicture}[
      ...
      \node[box, below=of clean] (target) {Create binary
        target:\texttt{ICD-10 O***} $\rightarrow$ 1, \texttt{ICD-10
          Z***}$\rightarrow$ 0};
      \node[box, below=of target] (profile) {AWS SageMaker Canvas Data
...
\end{tikzpicture}%
\end{adjustwidth}
\caption{Preprocessing workflow used to construct the \ac{HGIO}
...
\begin{table}[H]
  \small
% \centering
\caption{Summary of AWS SageMaker Canvas data-quality report before
final filtering.}
...
\begin{figure}[H]
  ...
  \end{adjustwidth}
  \caption{\label{fig:aws-wrangler}Data Wrangler is used to asses the
    quality of data before training the machine learning model. Low
    predictive power columns and duplicated records were
    removed. Resulting data is shuffle to prevent data leakage}
\end{figure}

\subsection{Statistical Characteristics of Collected Data}
\label{sec:stat-char-data}
We have analyzed the prevalence of \ac{PE} among the population
of pregnancies at the dataset provided by \ac{HGIO}. Some
of the important insights are that \ac{PE} prevalence rises
strongly with the patient's age: 85.13\%. The mean of women diagnosed
with \ac{PE} present a mean \ac{BMI} of 33. Normal pregnancies
lie in 25.71. These results are summarized in
...
of those variables organized in quartiles (data is split into 4
quantile groupes) \ac{w.r.t.} the
prevalence of \ac{PE} in pregnancies at \ac{HGIO}.
...
\begin{figure}[H]
  ...
  \caption{\label{fig:statistics}Statistical analysis of prevalence of
    preeclampsia in the population of pregnant women at \ac{HGIO}. Preeclampsia is more frequent in women of 36 to 49 years
  old and with a mean BMI of 33.}
\end{figure}
...
\begin{table}[H]
  ...
  preeclampsia diagnosis in pregnant women at \ac{HGIO}
...
\caption{Distribution of pregnancies and preeclampsia prevalence
  across demographic concentration quartiles in women at \ac{HGIO}
  from years 2022 to 20225.}
...
To complement the retrospective demographic dataset from \ac{HGIO} and compensate for
...
implemented in Python using \texttt{NumPy} and \texttt{pandas}, with a
fixed random seed for reproducibility.

The algorithm generates $n$ samples (default: $n=1000$) and first
...
from \ac{HGIO}. In Section~\ref{sec:materials-methods}, we
defined the \ac{PE} diagnose problem as a binary
classification. Consequently, this study used two complementary
training tracks: (i) a cloud-managed workflow in AWS SageMaker Canvas
...
classification, including Gradient Boosted Decision Trees (\texttt{XGBoost},
\texttt{LightGBM}, and \texttt{CatBoost}), \ac{RF}, \ac{MLP}, and regularized
\ac{LR}.
...
\begin{figure}[H]
  \begin{adjustwidth}{-\extralength}{0cm}
    ...
    \begin{tikzpicture}[node distance=6mm,box/.style={rectangle,
      ...
      {Cloud-managed workflow\\AWS SageMaker Canvas};
...
\textbf{Cloud-managed training (AWS SageMaker Canvas).}
AWS SageMaker Canvas is a managed visual machine-learning service that
supports data ingestion, preprocessing, model training, and model
evaluation through a no-code interface~\cite{aws_sagemaker_canvas_docs}.
In our workflow, Canvas was used for quality profiling,
feature screening support, and comparative model development under a
standardized cloud environment.

\textbf{Custom Automatic Machine Learning Workflow (C-AutoML)} We
proposed an iterative training methodology where different stratified
train/validation split are tested on every model candidate. The
training core executes an inner loop that iterates through various
algorithm families. For every model candidate within a specific
family, a hyperparameter configuration is stochastically sampled from
a predefined search space at each iteration. The proposed methodology
fits full sklearn pipelines with model-aware preprocessing (scaling
enabled for \ac{LR} and \ac{MLP}) and handles missing values using
imputation. Model selection is obtained by ranking the performance of
each classifier \ac{w.r.t.} \ac{F1}, primarily to account for
potential class imbalances, followed by \ac{AUC} and overall \ac{ACC},
which are the secondary and tertiary tie-breaking metrics,
respectively. Upon identifying the optimal pipeline, the system
performs a permutation-based feature importance analysis to enhance
model interpretability and provides comprehensive diagnostic
artifacts, including confusion matrices and performance logs.
...
cloud-managed workflow (AWS SageMaker Canvas), the target
...
provided data from Isidro Ayora Hospital (HGIO) and (ii) with the synthetic
clinical data that we produced. For the former, we trained our models using AWS Sage
Maker Canvas and our proposed methodology \ac{C-AutoML}. For the
latter, we only used \ac{C-AutoML}.
\subsection{Performance of C-AutoML on Isidro Ayora Hospital Data}
\label{sec:performance-c-automl}

Our proposed methodology \ac{C-AutoML} achieved its best performance
on model \texttt{LightGBM}, with an $\ac{F1}=0.802$,
$\mathtt{Precision}=0.837$, $\mathtt{Recall}=0.768$, and
$\ac{AUC}=0.944$, on Trial\#23,
...
\caption{C-AutoML performance across trials and iterations on the
  \ac{HGIO} dataset. Each trial uses a different
...
\caption[]{C-AutoML performance on \ac{HGIO} data (continued).} \\
\toprule
\textbf{Trial} & \textbf{Iter} & \textbf{Model} &
\textbf{F1} & \textbf{Acc} & \textbf{Prec} & \textbf{Rec} & \textbf{AUC} \\
\midrule
...
\begin{adjustwidth}{-\extralength}{0cm}
\centering
\subfloat[Normalized confusion matrix for the best \texttt{LightGBM} model
...
\subfloat[Mean and standard deviation of feature importance in the
  \texttt{LightGBM} prediction.\label{fig:feature-importance-lightgbm}]{
  \includegraphics[width=0.48\fulllength]{./figures/isidroayora_best_model_feature_effects_mean_std.png}}\\
\subfloat[Precision--recall curve for the best \texttt{LightGBM}
...
\end{adjustwidth}
\caption{Performance analysis of the best C-AutoML model (\texttt{LightGBM},
  Trial~23) on the \ac{HGIO} dataset. (\textbf{a})
...
\subsection{Performance of Weighted Ensemble Model on Isidro Ayora
  Hospital Data}
\label{sec:performance-aws-sage-maker-canvas}

Table~\ref{tab:canvas-model-performance} summarizes the performance of
the different models evaluated in AWS SageMaker Canvas. The best
performance was achieved by the a \texttt{Weighted Ensemble Model},
formed by \texttt{LightGBM}, \ac{MLP}, and \ac{RF} with an $\ac{F1}=0.805$,
$\mathtt{Precision}=0.770$, $\mathtt{Recall}=0.844$, and
$\ac{AUC}=0.956$. Compared to the \texttt{LightGBM} model trained with
...
the precision of \texttt{LightGBM} model implies a smaller false
positive rate; which is translated in less exams.
...
\begin{table}[H]
\caption{Performance of models evaluated in AWS SageMaker Canvas.
...
\subfloat[Normalized confusion matrix for the best Weighted Ensemble
  model (AWS SageMaker Canvas).\label{fig:confusion-matrix-bestmodel-aws}]{
...
\subfloat[KernelSHAP feature importance values for the AWS SageMaker
...
\subfloat[Precision--recall curve for the AWS SageMaker Weighted
...
\subfloat[F1-score comparison across all models trained in AWS
...
\end{adjustwidth}
\caption{Performance analysis of the AWS SageMaker Canvas Weighted
  Ensemble on the \ac{HGIO} dataset. (\textbf{a})
...
performance for model \ac{LR}, with a perfect score of $\ac{F1}=1.000$
on Trial\#1,
...
proposed models and iterations. It must be noticed that more than one
...
analyses: (i) applying SHAP to the LightGBM model trained on \ac{HGIO}
data, (ii) evaluating built-in feature importance from
AWS SageMaker Canvas, and (iii) applying SHAP to the best-performing
model on synthetic clinical data.

% \subsubsection{LightGBM on Isidro Ayora Hospital Dataset}
% \label{sec:lightgbm-isidro-ayor}
\subsubsection{SHAP Explainability of LightGBM on Isidro Ayora Hospital Dataset}
\label{sec:lightgbm-isidro-ayor}
...
show the feature importance \ac{w.r.t.} to the model output. For
...
supports that the LightGBM model captured demographic plausible risk
...
\caption{SHAP explainability visualizations for the best LightGBM
  model trained on the \ac{HGIO} dataset. (\textbf{a})
...
\subsection{SHAP Explainability of the Weighted Ensemble Model on
  Isidro Ayora Hospital Dataset}
\label{sec:ensemble-model-xai}

Figure~\ref{fig:canvas-shap-plots} presents the \ac{SHAP} analysis
for the \texttt{Weighted Ensemble} model trained in AWS SageMaker
Canvas on the \ac{HGIO} dataset, including a beeswarm
...
\subfloat[\ac{SHAP} beeswarm summary plot for the Weighted Ensemble
  ...
  and instance; colour encodes feature
...
\end{adjustwidth}
\caption{\ac{SHAP} explainability visualisations for the AWS SageMaker
  Canvas Weighted Ensemble model on the \ac{HGIO} dataset.
...
\subfloat[\ac{SHAP} beeswarm summary for the \ac{LR} model on
  ...
  for a single feature and instance; colour encodes feature
...
\end{adjustwidth}
\caption{\ac{SHAP} explainability visualisations for the best \ac{LR}
...
robust model for the prediction of \ac{PE}. For that matter, we
used the data provided by Isidro Ayora Hospital (HGIO). However, the digital records are most
demographic. Because of that, we generated a synthetic dataset using
variables commonly found in \ac{PIERS}.

For the \ac{HGIO} dataset we trained models with our
...
looks for improving the performance exposing the models to different
sets of train and validation data during each trial and also search
for best hyperparameter alignment. The best model found corresponded
to \texttt{LightGBM}. In parallel, we trained additional
machine learning models using AWS SageMaker Canvas, which produced as
best model a \texttt{Weighted Ensemble Model}. Finally, for the
clinical synthetic dataset, \ac{LR}, among other models, achieved
ideal performance.

Our results show that the \texttt{Weighted Ensemble Model} achieved
the best performance for the \ac{HGIO} dataset because it
has the highest recall ($\mathtt{Recall}=0.844$), compared to that of
\texttt{LightGBM} ($\mathtt{Recall}=0.768$). This is important,
because the recall (sensitivity) performance metric measures how many
of the actual population of women with \ac{PE}, the model
found. For a medical setting, it is necessary that the model has a
high recall because that lowers the false negatives. As a matter of
fact, the \texttt{Weighted Ensemble Model} loses 1 in 6 cases of
\ac{PE}. On the other hand, the \texttt{LightGBM} trained with
our proposed \ac{C-AutoML} approach has a higher precision
($\mathtt{Precision}=0.837$) compared to the \texttt{Weighted Ensemble
  model} ($\mathtt{Precision}=0.770$). This result is positive because
the higher the precision, the less false alarms. Consequently, it is
recommended to combine both of these models for prediction at the
health facility.

For the synthetic dataset, our results show an ideal performance due
to the controlled synthetic data generation. Whatsoever, the results
point \ac{LR} as a good fit for a \ac{PIERS} application.

The interpretability analysis, shows the feature importance of the
variables of each dataset towards prediction. For the \ac{HGIO} dataset, the most important features are: patient's age,
...
women at Isidro Ayora Hospital (HGIO). AWS SageMaker Canva's was pivotal in
assessing the prediction power of each of the features from the
provided dataset. Selecting features with a high prediction power
prevented our models to overfit, improving training. To the best of our
knowledge, this is the first work combining datasets of patients in
Ecuador and machine learning. Our results show that a combination of
our proposed methodology with the trained model from Canvas can lead
to effective predictions at the health facility.

Our proposed training methodology improved the hyperparamter settings
...
former accounts for the cost of false alarms. We aim towards recall
because it emphasizes minimizing missed diagnoses.

\texttt{LightGBM} achieved the best performance \ac{w.r.t.} the
\ac{F1} metric using our proposed \ac{C-AutoML} workflow, with a
$\ac{F1}=0.802$, $\mathtt{Recall}=0.768$, and
$\mathtt{Precision}=0.837$. This means that 77\% of true \ac{PE}
...
\texttt{Weighted Ensemble} model trained in AWS SageMaker achieved an
$\ac{F1}=0.805$, $\mathtt{Precision}=0.770$, and
$\mathtt{Recall}=0.844$. Therefore, 84\% of true \ac{PE} cases
...
to the ensemble model. We suggest a combination of both models models
to aid the physicians in their decision when diagnosing \ac{PE}
given the fact that the \texttt{Weighted Ensemble} model provides a
recall greater than 0.80, which can be considered clinically
reasonable if used alongside with clinical judgment and that
\texttt{LightGBM} achieves an improved precision that can reduce false
positives and improve resources administration. 

We look forward to improve our results in the near future, if
...
can help at health facilities in Ecuador. It must be noticed that our
models trained on the dataset from \ac{HGIO} use a few
demographic variables. This could help in a preventive diagnose in
such places where health facilities is scarce or expensive.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{Results}
...
\authorcontributions{Conceptualization, M.P.; methodology, M.P. and
  ...
  J.B.; supervision, M.P.; project administration, M.P.; validation,
...
\institutionalreview{The study was conducted in accordance with the
  ...
  EO-086-2024 (version~4). This study was designed as a retrospective,
  descriptive, cross-sectional observational study based exclusively on
  anonymized medical records provided by Isidro Ayora Hospital.}
...
\dataavailability{The anonymized patient dataset from Hospital
  Isidro Ayora used in this study is not publicly available due to
...
\acknowledgments{The authors gratefully acknowledge the financial
  ...
  through project PTT-22-03. We extend our sincere gratitude to
  Dr.~Jenniffer Andrea Calvopiña Medina and Dr.~Ramiro Felipe Ortiz
  Timbi, Zonal Health Coordinators of Zone~9 of the
  \textit{Ministerio de Salud Pública} (MSP), for their support in
  coordinating the necessary institutional approvals and data access
  for this study. Furthermore, we acknowledge the hospitals within
  Zone~9 for their interest in this project, as evidenced by their
  letters of support. In particular, we thank the \textit{Hospital
    Gineco Obstétrico Isidro Ayora} and its Director,
  Dra.~Teresa Natalia Aumala Viscarra, as well as
  Dr.~Santiago Vasco-Morales and the hospital's teaching unit and
  statistics unit, for providing access to the anonymized database of
  digital medical records and for their administrative and technical
...
HGIO    & Isidro Ayora Gynecology and Obstetrics Hospital \\
HTN     & Hypertension \\
...
\texttt{scikit-learn}~\cite{sklearn-api}, parameterised by the test
...
whether numeric feature standardisation is required.
...
\begin{itemize}
    ...
    \item \texttt{best\_model.joblib}: serialised optimal pipeline
...
\begin{itemize}
    ...
    \item \texttt{matplotlib} $\ge$ 3.9.4 — visualisation of training curves, confusion matrices, and \ac{SHAP} summary plots.
    \item \texttt{joblib} $\ge$ 1.0.0 — model serialisation (\texttt{.pkl} artefacts) and parallelised cross-validation loops.
\end{itemize}
...
models within the computing infrastructure of the \ac{EPN}. The system is currently accessible within the \ac{EPN} campus network and exposes the best-performing models produced by the \ac{C-AutoML} workflow and AWS Sage Maker Canvas procedure as a prediction service.
...
efficiently~\cite{mlflow_2018}. In this context, the \ac{MLOPs}
...
and serialised model artefact for every trial; and (iv)~deployment,
...
available from \ac{HGIO}, representing version~1 of the prediction
...
clearly labelled sections. The input variables presented in the
...
\subfloat[Gynaeco-obstetric history, medical history, and symptoms
