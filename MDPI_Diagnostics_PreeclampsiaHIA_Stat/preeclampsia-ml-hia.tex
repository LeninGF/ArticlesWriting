...
However, a major operational challenge for health records in Ecuador
...
dataset of patients at Isidro Ayora Hospital and to train a reference
...
gathered from Isidro Ayora Hospital for our study. However, we
...
a descriptive analysis of real hospital data from Isidro Ayora
Hospital, (ii) a cloud-managed benchmarking workflow, and (iii) a
...
\ac{PE} in women at Isidro Ayora Hospital. We restricted our
...
The records provided by the Isidro Ayora Third Level Gynecology and
Obstetrics Hospital contain a total of \numprint{174753} records 49
...
of pregnancies at the dataset provided by Isidro Ayora Hospital. Some
...
prevalence of \ac{PE} in pregnancies at Isidro Ayora Hospital.
...
\begin{figure}[H]
  ...
  \caption{\label{fig:statistics}Statistical analysis of prevalence of
    \ac{PE} in the population of pregnant women at Isidro Ayora
    Hospital. \Ac{PE} is more frequent in women of 36 to 49 years
...
\begin{table}[H]
  ...
  \ac{PE} diagnosis in pregnant women at Isidro Ayora Hospital
...
\caption{Distribution of pregnancies and preeclampsia prevalence
  across demographic concentration quartiles in women at Isidro Ayora
  Hospital from years 2022 to 20225.}
...
from Isidro Ayora Hospital. In Section~\ref{sec:materials-methods}, we
...
provided data from Isidro Ayora Hospital and (ii) with the synthetic
...
on model LightGBM, with an $\ac{F1}=0.802$,
...
\caption{C-AutoML performance across trials and iterations on the
  Isidro Ayora Hospital dataset. Each trial uses a different
...
\caption[]{C-AutoML performance on Isidro Ayora Hospital data (continued).} \\
\toprule
\textbf{Trial} & \textbf{Iter} & \textbf{Model} &
\textbf{F1} & \textbf{Acc} & \textbf{Prec} & \textbf{Rec} & \textbf{AUC} \\
\midrule
...
\caption{Performance analysis of the best C-AutoML model (LightGBM,
  Trial~23) on the Isidro Ayora Hospital dataset. (\textbf{a})
...
\caption{Performance analysis of the AWS SageMaker Canvas Weighted
  Ensemble on the Isidro Ayora Hospital dataset. (\textbf{a})
...
analyses: (i) applying SHAP to the LightGBM model trained on Isidro
Ayora Hospital data, (ii) evaluating built-in feature importance from
...
\end{adjustwidth}
\caption{SHAP explainability visualisations for the best LightGBM
  model trained on the Isidro Ayora Hospital dataset. (\textbf{a})
...
Canvas on the Isidro Ayora Hospital dataset, including a beeswarm
...
\caption{\ac{SHAP} explainability visualisations for the AWS SageMaker
  Canvas Weighted Ensemble model on the Isidro Ayora Hospital dataset.
...
used the data provided by Isidro Ayora Third Level Gynecology and
Obstetrics Hospital. However, the digital records are most
demographic. Because of that, we generated a synthetic dataset using
variables commonly found in \ac{PIERS}.

For the Isidro Ayora Hospital dataset we trained models with our
...
the best performance for the Isidro Ayora Hospital dataset because it
...
variables of each dataset towards prediction. For the Isidro Ayora
Hospital dataset, the most important features are: patient's age,
...
women at Isidro Ayora Hospital. AWS SageMaker Canva's was pivotal in
...
models trained on the dataset from Isidro Ayora Hospital use a few
