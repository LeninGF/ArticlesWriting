...
% To produce final clean copy: use make-final.py (see preeclampsia-ml-hia.tex)
...
\ac{PE}. \added[id=R1]{The synthetic dataset was constructed using the
...
\item \textbf{Final dataset:} After duplicate removal, feature
  ...
  weight (kg). \added[id=R1]{Table~\ref{tab:dataset-characteristics}
...
\begin{figure}[H]
  ...
  \begin{adjustwidth}{-\extralength}{0cm}
    \centering
    \includegraphics[width=\textwidth, height=.4\textheight]{./figures/WranglerPreeclampsia.drawio2.png}    %  figures/Aws-Wrangler-Operations.png
  \end{adjustwidth}
...
(Table~\ref{tab:variable_distribution}). Women diagnosed
...
\added[id=R2]{The record-level prevalence of \ac{PE} in the final
  dataset is 16.11\% (671 of 4,165 records), exceeding the global
  ...
  lower-prevalence settings. Table~\ref{tab:desc_stats} and
  Table~\ref{tab:variable_distribution} provide inter-group
...
\added[id=R12]{Although the $\mathrm{PE}_{\mathrm{FullPIERS}}$ score was used to
obtain an exploratory binary label with a threshold of $10\%$, empirical
validation studies have reported optimal cut-offs between $2\%\text{–}6\%$
...
which does not depend on SpO$_2$ or chest pain. Both variables are
...
% \added[id=R1]{The FullPIERS prognostic score was converted to a binary
...
synthetic data. \added[id=R1]{This is structurally expected: the diagnostic
...
\replaced[id=R1]{For the synthetic dataset, all six evaluated model
  ...
  inclusion of SpO$_2$, chest pain, platelet count, creatinine, and
  \ac{AST} in the synthetic track is grounded in the published
  FullPIERS equation (Equation~\eqref{eq:fullpiers-logit}), which uses
  precisely these variables as
  predictors~\cite{fullpiers_2011}. However, it must be noticed that
  the purpose of ~\eqref{eq:fullpiers-logit} is to predict adverse
  outcomes of \ac{PE}, which differs from our main purpose.}{For
